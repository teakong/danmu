...
if($type == "init") {
    ...
    // 返回给前端的白名单消息ID
    $messageIdList = isset($jsonObject["data"]["messageIdList"]) ? $jsonObject["data"]["messageIdList"] : array();
    ...
    if(empty($jsonObject) || !isset($jsonObject["data"]["messageIdList"]) || empty($jsonObject["data"]["messageIdList"])) {
        // 公开弹幕推送失败
        $resultModel["code"] = -1;
    ...
    // 填写你的deepseek API key, 速度最快推荐使用deepseek-v4-flash, 费率最低推荐使用千问百炼大模型: qwen3.8-flash
    $chatResult = requestAI($aiUrl, '', 'deepseek-v4-flash');


    // 降级为其他弹幕内容
    $head = "你正访问的网页标题是: ".$_GET["title"];
    if (!empty($chatResult['choices'][0]['message']['content'])) {
        $answer = trim($chatResult['choices'][0]['message']['content']);
        $head = $answer;
    }
    // 自定义发布人头像
    $avatar = "https://www.phprm.com/push/h5/static/avatar/danMu.png";
    ...
    $nextUrl = "http://www.phprm.com/services/push/trigger/${channelCode}?url=".rawurlencode($url)."&avatar=".rawurlencode($avatar)."&head=".urlencode($head). "&timestamp=".$timestamp. "&nonce=".$nonce. "&sign=".$sign;
    $json = file_get_contents($nextUrl);
    $jsonObject = json_decode($json, true);
